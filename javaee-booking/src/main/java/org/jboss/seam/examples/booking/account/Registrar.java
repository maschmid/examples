...
public class Registrar
{
   ...
   public void register()
   {
      if (verifyUsernameIsAvailable())
      {
         ...
         messages.info(new DefaultBundleKey("registration_registered")).defaults("You have been successfully registered as the user {0}! You can now login.").params(newUser.getUsername());
      }
      else
      {
   ...
   public void notifyIfRegistrationIsInvalid()
   {
      ...
      {
         messages.warn(new DefaultBundleKey("registration_invalid")).defaults("Invalid registration. Please correct the errors and try again.");
      }
   }
   ...
   private boolean verifyUsernameIsAvailable()
   {
      ...
      if (existing != null)
      {
         messages.warn(new BundleKey("messages", "account_usernameTaken")).defaults("The username '{0}' is already taken. Please choose another username.").targets(usernameInput.getClientId()).params(newUser.getUsername());
         return false;
      }
